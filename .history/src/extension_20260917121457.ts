...
async function fetchAllComments(token: string, org: string, repo: string, prNumber: number): Promise<GitHubComment[]> {
    const response = await fetch(
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    let seenCommentIds: number[] = context.globalState.get(`seenCommentIds_${repo}`, []);
    let unreadComments: GitHubComment[] = context.globalState.get(`unreadComments_${repo}`, []);

    const unreadStatusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 1001);
    ...
    const checkForNewComments = async () => {
        ...
        const newComments = comments.filter(c => {
            if (seenCommentIds.includes(c.id)) { return false; }
            if (allowedUsers.length > 0 && !allowedUsers.includes(c.user.login)) { return false; }
            return true;
        });
    ...
    const showUnreadList = async () => {
        ...
        if (action === 'View on GitHub') {
            ...
            unreadComments = unreadComments.filter(c => c.id !== selected.comment.id);
            persistState();
            updateUnreadStatusBar();
        }
    };
    ...
    const openSupportLinksCommand = vscode.commands.registerCommand(
        ...
        async () => {
            ...
            if (linkNames.length === 0) {
                vscode.window.showInformationMessage('Classroom 50: no support links configured.');
                return;
            }
    ...
    if (config['support-links'] && Object.keys(config['support-links']).length > 0) {
        const supportLinksStatusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 998);
        ...
        supportLinksStatusBarItem.command = 'classroom50-vscode-extension.openSupportLinks';
