...
import { fetchAdminOrgs, fetchFeedbackPR } from "./github";
...
async function setupTeacherFeatures(
  ...
  registerTeacherFeatures(context, token, adminOrgs);
  return true;
}
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (token && config && workspacePath) {
    tasks.push(setupStudentFeatures(context, token, config, workspacePath));
  }
