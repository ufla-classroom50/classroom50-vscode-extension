...
export async function registerStudentFeatures(
  ...
  const repoUrl = `https://github.com/${repoInfo.org}/${repoInfo.repo}`;
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(COMMANDS.submitAssignment, async () => {
      ...
      isSubmitting = true;
      try {
        await submitAndScheduleCheck(context, monitor, workspacePath, repoUrl);
      } finally {
        isSubmitting = false;
      }
    }),
